      ...
      <div id="calender" v-if="isCalendarDataPrepared">
        ...
        >
          ...
          <n-popover trigger="hover" v-for="(item, index) in getTodoList(year, month, date)" :key="index">
            ...
            <n-button v-if="item.is_finished === 0" quaternary type="primary" @click="changeFinish(item, 1)">
              完成
            </n-button>
            <span v-else-if="item.is_finished === 1" style="color: #18A058">
              已完成
            </span>
          </n-popover>
        </n-calendar>
...
function canlendarHandler () {
  ...
  const changeFinish = (item, flag) => {
    ...
    let updateStr = 'update todo_list set is_finished = ' + flag + ' where id = ' + item.id
